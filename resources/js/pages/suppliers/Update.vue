...
interface Props {
    show: boolean;
    supplierData?: SupplierInterface | null;
}
...
async function save(formData: any) {
    ...
    try {
        await SuppliersApi.update(props.supplierData!.id, formData.data);
        emit('updated');
...
<template>
    <Sheet :open="show" @update:open="(value) => $emit('update:show', value)">
        <SheetContent class="overflow-y-auto sm:max-w-2xl">
            <SheetHeader>
                <SheetTitle>Editar: {{ supplierData?.name }}</SheetTitle>
            </SheetHeader>
            <div class="p-3">
                <Form @submitted="save" :supplier="supplierData" ref="formComponent" />
            </div>
        </SheetContent>
    </Sheet>
</template>
